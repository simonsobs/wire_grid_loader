...
def powerOff(np05b=None,ports=None) :
    if np05b is None :
        if config.use_tcp:
...
def parseCmdLine(args):
    ...
    parser = OptionParser();
    parser.add_option('-p', '--ports', dest='ports', help='port list to be powered off (1-5). It can be specify multiple ports by combinding ports with "," (ex. "1,2").', type = str, default=None);
    (config, args) = parser.parse_args(args);
    if len(args)!=1 :
        print('ERROR! Wrong number of arguments ({}). Shold be 1.'.format(len(args)));
        print('       You should only use options specified by - or --.');
        print('       Please see `python powerOff.py --help`.');
        return 0;
    return config;
