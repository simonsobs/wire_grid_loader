from CHWPCollector_TCP import CHWPCollector;
...
def process_encoder_packet(parse_index):
    ...
    quad = int(quad_data)
    # Save the clock data
    #print('clock {}'.format(clock_data[0]));
    #print('overflow {}'.format(ovflw_data[0]));
    #print('overflow<<bits {}'.format(ovflw_data[0]<<num_overflow_bits));
...
collector = CHWPCollector(port,isTCP=isTCP);
