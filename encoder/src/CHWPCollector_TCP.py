...
class CHWPCollector(object):
    ...
    """
    def __init__(self, mcu_port=8080, isTCP=False):
        """
        ...
        self._process2 = multiprocessing.Process(
            target=self.relay_HWP_data,
            args=(self._should_stop, self._stopped, 0b1))
        self._process.start()
    ...
    def relay_HWP_data(self, should_stop, stopped, processID):
        ...
        while True:
            ...
            if self._pcount[processID]%1000 == 0 : print('process{} count in CHWPCollector = {}'.format(processID, self._pcount[processID]));
